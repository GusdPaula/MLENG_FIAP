...
output "reviewer_user_name" {
  value       = aws_iam_user.reviewer_user.name
  description = "Nome do usuário IAM de revisão (professor/avaliador)"
}

output "reviewer_user_arn" {
  value       = aws_iam_user.reviewer_user.arn
  description = "ARN do usuário IAM de revisão (professor/avaliador)"
}
