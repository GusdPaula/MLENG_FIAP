...
def _group_users_items(
    test_interactions: np.ndarray,
) -> dict[int, list[int]]:
    """Group test interactions by user, returning {user_idx: [item_ids]}."""
...
def hit_rate_at_k(
    ...
    hits = 0
    total = 0
    users_items = _group_users_items(test_interactions)

    with torch.no_grad():
        for user_idx, true_items in users_items.items():
            top_k_set = set(_get_user_top_k(model, user_idx, num_items, k, device))
...
def ndcg_at_k(
    ...
    with torch.no_grad():
        for user_idx, true_items in users_items.items():
            top_k_list = _get_user_top_k(model, user_idx, num_items, k, device)
            ndcg = _compute_user_dcg(top_k_list, true_items, k)
