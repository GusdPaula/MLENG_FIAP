...
class TelcoPipeline:
    ...
    def create_logistic_regression(
        self, class_weight: str = "balanced", max_iter: int = 1000
    ) -> Pipeline:
        ...
        pipeline = Pipeline(
            [
                ...
                (
                    "classifier",
                    LogisticRegression(
                        class_weight=class_weight, max_iter=max_iter, random_state=self.random_state
                    ),
                ),
            ]
        )
    ...
    def create_random_forest(
        self, n_estimators: int = 100, max_depth: int | None = 20, min_samples_split: int = 10
    ) -> Pipeline:
    ...
    def create_xgboost(
        self, n_estimators: int = 100, max_depth: int = 5, learning_rate: float = 0.1
    ) -> Pipeline:
